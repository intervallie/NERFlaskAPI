    ...
    if request.method == 'POST':
        ...
        if len(doc.ents) > 0:
            for ent in doc.ents:
                ...
                ORG_named_entity = df.loc[df['category'] == 'ORG']['value']# Subset semua entitas dengan kategori 'ORG'
                PERSON_named_entity = df.loc[df['category'] == 'PERSON']['value']# Subset semua entitas dengan kategori 'PERSON'
                GPE_named_entity = df.loc[df['category'] == 'GPE']['value'] # Subset semua entitas dengan kategori 'GPE'
                MONEY_named_entity = df.loc[df['category'] == 'MONEY']['value'] # Subset semua entitas dengan kategori 'MONEY'
                # END OF TASK 
...
def get_entities_normalizedfoo():
    ...
    for i in df['category'].unique():
        value = df.loc[df['category']==i]['value'].tolist()
        dictionary[i] = value
